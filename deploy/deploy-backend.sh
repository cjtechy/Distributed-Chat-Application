#!/bin/bash
# Idempotent production deploy — run on the logic EC2 instance (via GitHub Actions or manually).
set -euo pipefail
...
if [ ! -d "$APP_DIR/.git" ]; then
  log "ERROR: $APP_DIR is not a git checkout. Bootstrap with deploy/ec2-logic-userdata.sh first."
  exit 1
fi
